...
- **Configuration**: JSON files in `views/config/`
- **Styling**: Custom CSS variables in `public/theme.css`

## Project Structure

- `data/` - Data layer (Products, Shop Info, Nav)
- `server/app.js` - Express server, routes, dynamic config loading, and custom tags
...
Sections are self-contained components in `views/sections/`.
...
Layouts are driven by JSON files in `views/config/`.

- **Dynamic Loading**: `server/app.js` loads all JSON files in `views/config/` and exposes them globally as `[filename]Group` (e.g., `header.json` -> `headerGroup`).
- **Structure**:
  ```json
  {
    "type": "header",
...
1. Create a file in `views/sections/` (e.g., `my-section.psp`).
